 ...
 */
export function thinSampleFloor(lengthDays: number): number {
  return 2 * lengthDays;
}

export interface ProseRecommendation {
  /** 1-based, largest lever first. */
  rank: number;
  title: string;
  body: string;
  /** Mono evidence lines. Every figure comes from the findings JSON. */
  evidence: string[];
  /** The size cell: a number, `Unknown`, or `Held`. */
  size: string;
  sizeLabel: string;
}

export interface ProseRemeasure {
  date: string;
  command: string;
  targets: string;
}
...
export interface ReportProse {
  ...
  headline: string;
  conclusion: string;
  /** Second paragraph: what did not improve, and where the savings sit. */
  outlook: string;
  recommendations: ProseRecommendation[];
...
export interface ProseOptions {
  /** `YYYY-MM-DD` the re-measure date is counted from. Defaults to the clock. */
  today?: string;
}
...
function move(row: HabitsScorecardRow): string {
...
export function topLever(report: HabitsReport): HabitsLever | null {
  if (!report.levers.length) return null;
...
function leverAim(lever: HabitsLever): { share: number; amount: number; hedged: boolean } {
...
function heaviestBand(window: HabitsWindow) {
  return window.byDuration.reduce(
    (top, band) => (band.costShare > top.costShare ? band : top),
    window.byDuration[0],
  );
}
 ...
 */
function thinWindows(report: HabitsReport, lengthDays: number): string[] {
  const floor = thinSampleFloor(lengthDays);
  const thin: string[] = [];
  if (report.current.sessions < floor) thin.push(`the current window (${n(report.current.sessions)} sessions)`);
...
function conclusionText(report: HabitsReport, unit: string): string {
  ...
  if (headline.finding === 'single-window' || !prior) {
    parts.push(
      `The window covers ${n(current.prompts)} prompts across ${n(current.sessions)} sessions ` +
  ...
  parts.push(
    `Sessions carrying a behavioural flag hold ${current.flagged.costShare}% of the window ` +
  ...
  if (headline.finding === 'volume') {
    parts.push(
      'Cost tracking volume is the result that means there is no habit to fix here — ' +
    ...
    if (worse.length) {
      parts.push(
        `The rows moving the wrong way are ${worse
          .slice(0, 2)
    ...
    parts.push(
      `${improved} of ${report.scorecard.length} measures improved and ${worse} moved the wrong ` +
...
function outlookText(report: HabitsReport, unit: string, lengthDays: number): string {
  ...
  if (lever) {
    ...
    parts.push(
      `The remaining savings sit in ${lever.basis} — ${n(lever.ceiling)} ${unit} at the ceiling ` +
        `(${lever.ceilingShare}% of the window)` +
        (aim.hedged ? `, of which ${n(aim.amount)} (${aim.share}%) is the band worth aiming at.` : '.'),
    );
  } else {
    parts.push(
      'No lever in this data sizes above zero: on the measures ccalyze can see, the sessions ' +
  ...
  if (!current.cleanCohortUsable) {
    parts.push(
      `Unflagged sessions hold only ${current.clean.promptShare}% of prompts, too small a cohort ` +
  ...
  if (thin.length) {
    parts.push(
      `Read single rows as anecdote rather than direction: ${thin.join(' and ')} sits under the ` +
...
function leverRecommendation(report: HabitsReport, unit: string): ProseRecommendation {
  const lever = topLever(report);
 ...
 */
function baselineRecommendation(report: HabitsReport): ProseRecommendation {
  return {
    rank: 2,
...
function heldRecommendation(report: HabitsReport): ProseRecommendation {
...
function durationCaptionText(report: HabitsReport, unit: string): string {
  const { current, prior } = report;
  ...
  return (
    `Sessions of ${top.band} carry ${top.costShare}% of the window's ${unit} across ` +
 ...
 */
export function buildProse(report: HabitsReport, options: ProseOptions = {}): ReportProse {
