...
export function buildHabitsReport(
  ...
  const report: HabitsReport = {
    ...
    caveats: {
      costIsNotional:
        'ccalyze prices tokens at published per-token API list rates, which is ' +
        'explicitly not what a subscription charges. Treat the figure as a quota ' +
        'proxy, never as spend.',
      durationIsWallClock:
        'Session duration runs from first to last message, so idle and overnight ' +
        'time counts. It is not a working-hours measure.',
      flaggedShareIsHigh:
        'no-compaction fires at 30 prompts and long-running at three hours, so ' +
      ...
      baselineUnmeasured:
        'Per-request baseline (MCP tool definitions plus instruction files) is not ' +
