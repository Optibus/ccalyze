 ...
 */
export function deriveSessionName(display) {
    const oneLine = display.replace(/\s+/g, ' ').trim();
    if (oneLine.length <= SESSION_NAME_MAX_CHARS)
        return oneLine;
 ...
 */
export function computeColdStarts(messages) {
    ...
    for (let i = 1; i < ordered.length; i++) {
        ...
        if (rebuiltTokens < COLD_START_MIN_CACHE_WRITE_TOKENS)
            continue;
        const gapMinutes = (Date.parse(msg.timestamp) - Date.parse(ordered[i - 1].timestamp)) / 60_000;
        if (!(gapMinutes > COLD_START_GAP_MINUTES))
            continue;
...
function firstDisplayBySession(history) {
...
function capPromptDisplay(display) {
    if (display.length <= DEEP_MAX_PROMPT_DISPLAY_CHARS)
        return display;
 ...
 */
export function groupTranscriptsByDir(paths) {
    const byDir = new Map();
...
export function aggregate(sessions, history, range, deep = false) {
    ...
    for (const session of sessions) {
        ...
        for (const [model, tokens] of modelTokensInSession) {
            const existing = dayEntry.tokensByModel[model] ?? {
                input: 0,
                output: 0,
                cacheRead: 0,
                cacheWrite: 0,
    ...
    const byProject = Array.from(byProjectMap.values()).sort((a, b) => b.costUSD - a.costUSD);
    // Sort days by date ascending
    const byDay = Array.from(byDayMap.values()).sort((a, b) => a.date.localeCompare(b.date));
    const byModel = Object.fromEntries(byModelMap);
    const output = {
        range,
        summary,
        byDay,
 ...
 */
export function buildDeepData(sessions, summaries, history) {
    ...
    const deepSessions = sessions.map((session) => {
        const summary = summaryById.get(session.sessionId);
        // Model timeline: messages sorted by time, consecutive duplicates collapsed.
        const modelTimeline = [];
        const sortedMessages = [...session.messages].sort((a, b) => a.timestamp.localeCompare(b.timestamp));
        ...
        for (const msg of sortedMessages) {
            if (msg.isSidechain)
                continue;
            if (lastMainModel !== undefined && lastMainModel !== msg.model)
    ...
    return {
        note: 'Deep index for insights fusion. For each session below, read its transcript(s) — ' +
