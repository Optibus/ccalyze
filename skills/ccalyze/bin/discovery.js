 ...
 * once per alias.
 */
export function discoverSessionFiles(claudeDir, from, to) {
    ...
    const collect = (filePath, project) => {
        let fstat;
        try {
        ...
        if (fstat.mtime < from || fstat.mtime > toEnd)
            return;
        const physicalKey = `${fstat.dev}:${fstat.ino}`;
        if (seenFiles.has(physicalKey))
            return;
...
function mergeUsage(existing, incoming) {
    ...
    a.cache_creation_input_tokens = Math.max(a.cache_creation_input_tokens, b.cache_creation_input_tokens);
 ...
 * list by hand: message-level data merges by `requestId`, but a scalar counted
 * once per transcript is invisible to that merge and would silently keep only
...
export function mergeSessions(sessions) {
    ...
    for (const session of sessions) {
        ...
        if (!entry) {
            ...
            for (const msg of session.messages) {
                ...
                if (seen)
                    ...
                    byRequest.set(msg.requestId, msg);
            }
        ...
        if (session.startTime < target.startTime)
            target.startTime = session.startTime;
        if (session.endTime > target.endTime)
            target.endTime = session.endTime;
    }
