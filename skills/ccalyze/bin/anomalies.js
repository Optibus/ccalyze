import { isPricingKnown } from "./cost.js";
/**
...
export function detectAnomalies(output) {
    const anomalies = [];
    ...
    const poorlyCached = output.sessions
        .filter((s) => s.costUSD >= LOW_CACHE_MIN_COST_USD && s.cacheReadRatio < LOW_CACHE_RATIO_THRESHOLD)
    ...
    for (const session of poorlyCached) {
        anomalies.push({
            ...
            severity: 'medium',
            sessionId: session.id,
            detail: `Only ${(session.cacheReadRatio * 100).toFixed(1)}% of this $${session.costUSD.toFixed(2)} ` +
    ...
    for (const session of rebuilders) {
        anomalies.push({
            ...
            severity: 'medium',
            sessionId: session.id,
            detail: `Rebuilt its context cache from cold ${session.coldStarts} times after long idle gaps, ` +
                `a $${session.coldStartExtraUSD.toFixed(2)} premium over what a warm cache would have cost`,
        });
    }
    // Concurrent opus sessions
    const opusSessions = output.sessions.filter(s => s.primaryModel.includes('opus') && s.startTime && s.endTime);
    ...
    if (output.deep) {
        const churny = output.deep.sessions
            .filter((s) => s.mainModelSwitchCount >= MAIN_CHURN_SWITCH_THRESHOLD && s.flags.includes('high-cost'))
        ...
        for (const session of churny) {
            anomalies.push({
                ...
                severity: 'medium',
                sessionId: session.id,
                detail: `${session.mainModelSwitchCount} main-thread model switches in a ` +
    ...
    if (unpriced.length > 0) {
        anomalies.push({
            ...
            severity: 'medium',
            detail: `No published pricing for ${unpriced.join(', ')} — cost estimated at Opus rates. ` +
